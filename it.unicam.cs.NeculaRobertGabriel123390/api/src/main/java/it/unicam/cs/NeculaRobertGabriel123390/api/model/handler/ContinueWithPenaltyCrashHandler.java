...
import it.unicam.cs.NeculaRobertGabriel123390.api.model.log.ContinueWithPenaltyCrashLog;
 ...
 * This handler lets the user continue the race, resetting its moves
 */
public class ContinueWithPenaltyCrashHandler implements RaceHandler {
    ...
    public void logEvent(RaceManager raceManager, Position newPosition) {
        raceManager.getEventLogger().logRaceEvent(new ContinueWithPenaltyCrashLog(raceManager.getCurrentPlayer(), newPosition));
    }


    /**
     * Method that updates the state of the player, resetting its moves and continuing the race
     * @param raceManager - The race manager
     */
    private void updateState(RaceManager raceManager) {
        raceManager.getPlayersManager().getPlayerMoves(raceManager.getCurrentPlayer()).resetMoves();
